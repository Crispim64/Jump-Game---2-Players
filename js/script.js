...
pipe1.addEventListener("animationiteration", () => {
  if (!isGameOver) {
    ...
    if (audioCtx) {
      ...
      if (noiseGainNode && noiseGainNode.gain.value < 0.6) {
        noiseGainNode.gain.value += 0.03;
      }
    }
  }
});
...
const stopAnimations = () => {
  const animatedElements = document.querySelectorAll(".pipe, .mountain-fg, .mountain-bg");
  ...
  clouds.forEach((cloud) => {
    cloud.getAnimations().forEach((anim) => {
      anim.playbackRate = 0.15;
    });
  });
};
...
const handleDeath = (loser, loserPipe, imgOver, winnerName, isMario) => {
  ...
  if (isMario) {
    gameOverMarioSnd.play();
  } else {
    gameOverLuigiSnd.play();
  }
